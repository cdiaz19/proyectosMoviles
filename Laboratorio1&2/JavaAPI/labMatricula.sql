...
-- COURSEs by Carrer
CREATE OR REPLACE FUNCTION listarCursosCarrera(idCarrera IN carrera.id%TYPE)
RETURN Types.ref_cursor
AS
  cursorCurso types.ref_cursor;
BEGIN
  OPEN cursorCurso FOR
    SELECT lc.id, lc.anno, ca.nombre, cu.nombre, ci.numero from listaCurso lc join carrera ca on lc.carreraId = ca.id join ciclo ci on lc.cicloId = ci.id join curso cu on lc.cursoId = cu.id where lc.carreraId=idcarrera;
  RETURN cursorCurso;
END;
/
