...
public class VideoJuegoAdapter extends RecyclerView.Adapter<VideoJuegoAdapter.MyViewHolder> implements Filterable {

    private VideoJuegoAdapterListener listener;
    private VideoJuego deletedItem;
    private List<VideoJuego> grupoListFiltered;
    private List<VideoJuego> grupoList;

    public VideoJuegoAdapter(List<VideoJuego> grupolist, VideoJuegoAdapterListener listener){
    ...
    @Override
    public VideoJuegoAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(VideoJuegoAdapter.MyViewHolder holder, int position) {
        final VideoJuego grupo = grupoListFiltered.get(position);
        holder.titulo1.setText(grupo.getCodigo());
        holder.titulo2.setText(grupo.getNombre());
        holder.description.setText(String.valueOf(grupo.getPrecio()));
    }
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    for (VideoJuego row : grupoList) {
                        // filter use two parameters
                        if (row.getCodigo().toLowerCase().contains(charString.toLowerCase()) || row.getCategoria().getNombre().toLowerCase().contains(charSequence)) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                grupoListFiltered = (ArrayList<VideoJuego>) filterResults.values;
    ...
    public void removeItem(int position) {
        ...
        Iterator<VideoJuego> iter = grupoList.iterator();
        while (iter.hasNext()) {
    ...
    public interface VideoJuegoAdapterListener {
        void onContactSelected(VideoJuego grupo);
    }
}
