...
public class AdmOrderActivity extends AppCompatActivity implements RecyclerItemTouchHelper.RecyclerItemTouchHelperListener, OrderAdapter.OrderAdapterListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    final Type tipoListaOrdenes = new TypeToken<List<Order>>(){}.getType();
    final List<Order> orders = gson.fromJson(json, tipoListaOrdenes);


    mRecyclerView = findViewById(R.id.recycler_carrerasFld);
    ordersList = new ArrayList<>();
    model = new ModelData(null, null, null, orders);
    ordersList = model.getOrderList();
    mAdapter = new OrderAdapter(ordersList, this);
  ...
  private void goToAddUpdOrder() {
    Intent intent = new Intent(this, AddUpdOrderActivity.class);
  ...
  private void checkIntentInformation() {
    ...
    if (extras != null) {
      Order aux;
      aux = (Order) getIntent().getSerializableExtra("addOrder");
      if (aux == null) {
        aux = (Order) getIntent().getSerializableExtra("editOrder");
        if (aux != null) {
          //found an item that can be updated
          boolean founded = false;
          for (Order order : ordersList) {
            if (order.getFecha().equals(aux.getFecha())) {
              order.setFecha(aux.getFecha());
              founded = true;
              break;
            }
          }
          //check if exist
          if (founded) {
            Toast.makeText(getApplicationContext(), "Order: "  + aux.getClient().getNombre() + " edited!", Toast.LENGTH_LONG).show();
          } else {
            Toast.makeText(getApplicationContext(), "Order: "  + aux.getClient().getNombre() + " not finding!", Toast.LENGTH_LONG).show();
          }
        }
      } else {
        //found a new Category Object
        ordersList.add(aux);
        Toast.makeText(getApplicationContext(), aux.getFecha() + " added to List!", Toast.LENGTH_LONG).show();
      }
    }
  }
  ...
  public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction, int position) {
    if (direction == ItemTouchHelper.START) {
      if (viewHolder instanceof OrderAdapter.MyViewHolder) {
        ...
        String name = ordersList.get(viewHolder.getAdapterPosition()).getFecha();
        tempUrl = apiUrl + "deleteOrder?fecha="+name;
        MyAsyncTasks myAsyncTasks = new MyAsyncTasks();
      ...
      Order aux = mAdapter.getSwipedItem(viewHolder.getAdapterPosition());

      //send data to Edit Activity
      Intent intent = new Intent(this, AddUpdOrderActivity.class);
      intent.putExtra("editable", true);
      intent.putExtra("order", aux);
  ...
  @Override
  public void onContactSelected(Order order) { //TODO get the select item of recycleView
    Toast.makeText(getApplicationContext(), "Selected Order: " + order.getFecha(), Toast.LENGTH_LONG).show();
  }
