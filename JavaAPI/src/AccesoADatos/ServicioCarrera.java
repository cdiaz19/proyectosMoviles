...
public class ServicioCarrera extends Servicio {
    ...
    private static final String LISTARCURSOSCARRRERA = "{?=call listarCursosCarrera(?)}";
    ...
    throws GlobalException, NoDataException
  {
    ...
    public LinkedList<ListaCurso>  listarCursosCarrera(String nombre)
            throws GlobalException, NoDataException {
        try {
            conectar();
        } catch (ClassNotFoundException e) {
        ...
        ResultSet rs = null;
        LinkedList<ListaCurso> coleccion = new LinkedList();
        CallableStatement pstmt = null;
        try {
            pstmt = this.conexion.prepareCall(LISTARCURSOSCARRRERA);
            pstmt.registerOutParameter(1, -10);  
            pstmt.setString(2, nombre);
            pstmt.execute();
            //id, anno, carreraId, cursoId, cicloId
            rs = (ResultSet) pstmt.getObject(1);
