...
public class AdmSeguridadActivity extends AppCompatActivity implements RecyclerItemTouchHelper.RecyclerItemTouchHelperListener, SeguridadAdapter.SeguridadAdapterListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        model = new ModelData();
        usuarioList = model.getUsuariosList();
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction, int position) {
        if (direction == ItemTouchHelper.START) {
            if (viewHolder instanceof SeguridadAdapter.MyViewHolder) {
                ...
                String name = usuarioList.get(viewHolder.getAdapterPosition()).getCorreo();
    ...
    public void onContactSelected(Usuario user) { //TODO get the select item of recycleView
        Toast.makeText(getApplicationContext(), "Selected: " + user.getCorreo() + ", " + user.getPrivilegio(), Toast.LENGTH_LONG).show();
    }
                ...
                if (aux != null) {
                    ...
                    for (Usuario usuario : usuarioList) {
                        if (usuario.getCorreo().equals(aux.getCorreo())) {
                            usuario.setPrivilegio(aux.getPrivilegio());
                            usuario.setContraseña(aux.getContraseña());
                            founded = true;
                            break;
                        }
                    }
                    //check if exist
                    if (founded) {
                        Toast.makeText(getApplicationContext(), aux.getCorreo() + " editado correctamente", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(getApplicationContext(), aux.getCorreo() + " no encontrado", Toast.LENGTH_LONG).show();
                    }
                }
            } else {
                //found a new Profesor Object
                usuarioList.add(aux);
                Toast.makeText(getApplicationContext(), aux.getCorreo() + " agregado correctamente", Toast.LENGTH_LONG).show();
            }
        }
    }
